...
console.log("Hello there! Im here. - from [app/speedtestWORKER] worker.");
...
var client_ip = ""; // client's IP address as reported by getIP.php
var client_networkType = "";
var client_hostname = "";
var client_city = "";
var client_region = "";
var client_country = "";
var client_org = "";
var client_org_cutted = "";
...
var settings = {
	...
	url_telemetry: "https://oracle.api.deqstudio.com/speedtest_app/telemetry/register", // path to the script that adds telemetry data to the database
	...
	if (params[0] === "status") {
		...
		postMessage(
			JSON.stringify({
				...
				client_networkType: client_networkType,
				client_ip: client_ip,
				...
				client_org_cutted: client_org_cutted,
	...
	xhr.onload = function() {
		...
		//try {
			...
			if(typeof connection !== 'undefined'){
				//console.log("CONN_1");
				client_networkType = connection.type;
				if(client_networkType == "wifi"){
					client_networkType = "Wi-Fi";
					//console.log("CONN_2")
				}else if(client_networkType == "cellular"){
					//console.log("CONN_3")
					if(connection.type != null && connection != undefined){
						//console.log("CONN_5")
						client_networkType = connection.effectiveType.toUpperCase();
					}
				}else{
					if(typeof connection.type == 'undefined'){
						//console.log("CONN_6")
						client_networkType = "n/A";
					}
				}
			}else{
				client_networkType = "n/A";
			}
			client_ip = data.rawIspInfo.ip;
			//console.log("demoA: " + client_ip);
			client_hostname = data.rawIspInfo.hostname;
			client_city = data.rawIspInfo.city;
			client_region = data.rawIspInfo.region;
			client_country = data.rawIspInfo.country;
			client_org = data.rawIspInfo.org;
			client_org_cutted = client_org.substr(client_org.indexOf(' ') + 1);
			ispInfo = data.rawIspInfo;
		//} catch (e) {
			//console.log("demoAe: " + client_ip)
...
function sendTelemetry(done) {
	...
	xhr.onload = function() {
		try {
			...
			if (jsonResponse["insertId"] > 0) {
				try {
					var id = jsonResponse["insertId"];
					done(id);
				} catch (e) {
					done(null);
				}
	...
	xhr.open("POST", settings.url_telemetry);
	...
	var postData = "dl=" + encodeURIComponent(dlStatus) + 
	"&ul=" + encodeURIComponent(ulStatus) +
	"&ping=" + encodeURIComponent(pingStatus) +
	"&jitter=" + encodeURIComponent(jitterStatus) +
	"&provider=" + encodeURIComponent(client_org_cutted) +
	"&ip=" + encodeURIComponent(client_ip) +
	"&hostname=" + encodeURIComponent(client_hostname) +
	"&city=" + encodeURIComponent(client_city) +
	"&region=" + encodeURIComponent(client_region) +
	"&country=" + encodeURIComponent(client_country) +
	"&pixelVersion=" + encodeURIComponent("1.0.2e-avnet");
